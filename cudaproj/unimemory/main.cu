...
public:
  ...
  {
    printf("Assignment constract..char \n");
    _realloc(strlen(s));
    strcpy(data, s);
    return *this;
  }
  ...
  __host__ __device__ char &operator[](int pos) { return data[pos]; }

  // C-string access
  __host__ __device__ const char *c_str() const { return data; }
